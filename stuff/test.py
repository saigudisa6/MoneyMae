...
def createAll(data):
    ...
    for x in range(rowIndex - 1):
        buyers.append(User(int(data[x][0]),float(data[x][1]),float(data[x][2]),float(data[x][3]),float(data[x][4]),
                    float(data[x][5]),float(data[x][6]),float(data[x][7]),float(data[x][8]),int(data[x][9])))
    return buyers


print(createAll(data) + "\n")
